...
def index(request):
    if request.method == "POST":
        ...
        if food_consumed == "--Select Food--":
            ...
            foods = Food.objects.filter(name=food_consumed)
            if foods.exists():
                # You can choose to get the first item or handle duplicates
                consume = Consume(user=request.user, food_consumed=foods.first())
                consume.save()
            else:
                messages.error(request, "Food not found.")  # Handle case when food is not found

            # Fetch foods associated with the current user
